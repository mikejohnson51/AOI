...
To define a bounding box **clip**,  requires one to four, inputs given as a list:
...
All **clip** inputs **must** be provided as a list and order **DOES** matter if each parameter is not explicitly assigned (eg `h = 10`). Examples for each input follow: 

##1. Location

A Location is the first input in a  **clip** list and can be given as a colloquial name (*character string*) or as a (lat/long) pair (*numeric*). Names are geocoded via the OpenStreet map API as shown [here](articles/geocode.html).

If only a location is provided the OSM derived bounding box is used, this is also the only instance where clip inputs do not have to be in list form: 

```{r}
AOI = getAOI(clip = "Sacramento")
```

```{r, echo = FALSE}
leaflet(option=leafletOptions(zoomControl= FALSE, minZoom = 9, maxZoom = 9)) %>% addProviderTiles(providers$CartoDB.Positron) %>% 
  addPolygons(data = AOI,fillColor = "transparent", color = col, weight = 4, label = "Sacramento")  
```
...
```{r, echo = FALSE}
nwc = geocode("National Water Center, AL")
...
Returning to the Grand Caynon example, if the W and H are not specified, getAOI() does its best to generate an AOI from the OSM geocoder. 

```{r}

GC = getAOI(clip = "garden of the gods, colorado springs")

```

```{r}
leaflet(option=leafletOptions(zoomControl= FALSE, minZoom = 13, maxZoom = 13)) %>% addTiles() %>% 
   addPolygons(data = GC, fillColor = "transparent", color = col, weight = 4)  
