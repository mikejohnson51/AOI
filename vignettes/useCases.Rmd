...
AOI = getAOI(list("UCSB", 10, 10, "lowerleft")) %>% 
  HydroData::findNHD() %>% 
  HydroData::findWaterbodies() %>% 
  HydroData::findNWIS()

leaflet() %>% addTiles() %>% addPolylines(data = AOI$nhd, weight = AOI$nhd$streamorde) %>% 
  addMarkers(data = AOI$nwis) %>% 
  addPolygons(data = AOI$waterbodies, weight = 1) %>% addPolygons(data = AOI$AOI, color = 'red', fillColor = "transparent")
...
AOI = getAOI(clip = list("UCSB", 10, 10)) %>% HydroData::findElev()

ned = matrix(raster::extract(AOI$elev,raster::extent(AOI$elev),buffer=1000),
               nrow=ncol(AOI$elev),ncol=nrow(AOI$elev))
...
ned %>%
  ...
  save_png(filename = paste0(getwd(), "/rayshade_ex.png"))
...
files = nwm::getFilelist(type = 'land',  startDate = Sys.Date()-3, t = 0, f = c(3,6,9,12))

AOI = getAOI(list("Devils Tower, WY", 10, 10)) 

nwm = nwm::downloadNWM(AOI, files, param = 'accet')
...
```{r}
AOI = getAOI(list("Boulder", 10, 10)) %>% as_Spatial %>%  FedData::get_ssurgo(label = "test")
...
AOI = getAOI(list('UCSB', 2, 2)) %>% bbox_st()
...
```{r, message=F}
library(rosm)
bb = getAOI(clip = list("Lake Cachuma", 20,20)) %>% sf::as_Spatial() %>% 
  osm.plot(type="stamenbw")
```


#USGS-R Geoknife

The USGS geoknife package provides a unique interface to large-scale climate datasets. One example of these is the LOCA Downscaled climate product. Here, we are interested in finding predicted maximum daily temperture values for the Santa Barbara area in October of 2080 under two climate scenarios - RCP 4.5 and 8.5. This example is basic and developed to highlight the use of AOI with the geoknife package. A more in depth example of the geoknife package can be found [here](https://owi.usgs.gov/blog/LOCAdownscaling/).
...
AOI = getAOI("Santa Barbara")

AOI::check(AOI, returnMap = TRUE)
