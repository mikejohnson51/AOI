...
getAOI = function(clip = NULL, country = NULL, state = NULL, county = NULL,
...
# Error Catching
  ...
  if (is.null(country)) {
    ...
    if (!is.null(state)) {
      ...
      if (!is.null(clip)) { stop("Only 'state' or 'clip' can be used. Set the other to NULL") }
      ...
      if ( is.null(clip))   { stop("Requires a 'clip' or 'state' parameter to execute.")}
    }
  }
...
# Fiat Boundary Defintion (Exisiting Spatial/Raster Feature or getFiat())
  ...
  } else if (checkClass(clip, c('raster', 'spatial', 'sf'))) {
    st_transform(getBoundingBox(clip), aoiProj)
  } else {
    getClip(clip, km)
  }
